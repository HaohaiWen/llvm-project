...
void ObjDumper::printBBAddrMapFunction(StringRef FuncName,
                                       const object::BBAddrMap &AM,
                                       const object::PGOAnalysisMap &PAM,
                                       bool PrettyPGOAnalysis) {
  W.printString("Name", FuncName);
  {
    ListScope BBRL(W, "BB Ranges");
    for (const object::BBAddrMap::BBRangeEntry &BBR : AM.BBRanges) {
      DictScope BBRD(W);
      W.printHex("Base Address", BBR.BaseAddress);
      ListScope BBEL(W, "BB Entries");
      for (const object::BBAddrMap::BBEntry &BBE : BBR.BBEntries) {
        DictScope BBED(W);
  ...
  if (PAM.FeatEnable.hasPGOAnalysis()) {
    ...
    if (PAM.FeatEnable.hasPGOAnalysisBBData()) {
      ...
      for (const object::PGOAnalysisMap::PGOBBEntry &PBBE : PAM.BBEntries) {
        DictScope L(W);
